  ...
  <div class="app-shell">
    <header>
      <div class="header-row">
        ...
        <nav :class="{ open }"><RouterLink v-for="item in nav" :key="item[1]" :to="item[1]" @click="open=false"><component :is="item[2]" :size="15"/><span>{{ item[0] }}</span></RouterLink></nav>
    ...
    <div class="mobile-nav"><RouterLink v-for="item in nav.slice(0,5)" :key="item[1]" :to="item[1]"><component :is="item[2]" :size="19"/><small>{{ item[0] }}</small></RouterLink></div>
  </div>
</template>
