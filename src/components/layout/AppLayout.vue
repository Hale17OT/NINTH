...
const activeSport = computed(() =>
  sportId.value === "platform" ? null : sportById(sportId.value),
);
const nav = computed(() =>
  activeSport.value?.id === "baseball"
    ? sportNavigation.baseball
    : activeSport.value
      ? (sportNavigation[activeSport.value.id] || sportNavigation.default)(
          activeSport.value,
        )
      : [
          ["Platform", "/"],
          ["All-sports builder", "/build"],
        ],
);
...
const quickResults = computed(() =>
  Object.entries(searchResults.value)
    .flatMap(([group, items]) =>
      (items || []).slice(0, 4).map((item) => ({ ...item, group })),
    )
    .slice(0, 9),
);
const gamePath = (game) =>
  /live|progress|delay/i.test(game.status || "")
    ? `/live/${game.id}`
    : `/games/${game.id}`;
const tickerState = (game) =>
  /live|progress|delay/i.test(game.status || "")
...
const closeOutside = (event) => {
  if (!searchRoot.value?.contains(event.target)) focused.value = false;
};
...
watch(query, (value) => {
  ...
  if (!isBaseball.value || term.length < 2) {
    searchResults.value = { Teams: [], Players: [], Games: [] };
  ...
  searchTimer = setTimeout(async () => {
    try {
      searchResults.value = await api.search(term);
    } catch {
      searchResults.value = { Teams: [], Players: [], Games: [] };
    } finally {
      searching.value = false;
...
<template>
  <div
    class="app-shell"
    :class="[`workspace-${sportId}`]"
    :style="activeSport ? { '--workspace-accent': activeSport.accent } : {}"
  >
    <motion.div
    ...
    <header class="workspace-header">
      <div class="header-row">
        <RouterLink to="/" class="brand" aria-label="NINTH home"
          ><img src="/brand/ninth-icon-192.png" alt="" /><span
            >NINTH</span
          ></RouterLink
        >
        <nav class="sport-nav" aria-label="Sport workspaces">
          <RouterLink to="/" :class="{ active: sportId === 'platform' }"
            ><span>00</span><b>Platform</b
            ><SharedIndicator
              v-if="sportId === 'platform'"
              layout-id="sport-nav"
          /></RouterLink>
          <RouterLink
            ...
            :style="{ '--sport': sport.accent }"
            ><span>{{ sport.numeral }}</span
            ><b>{{ sport.short }}</b
            ><SharedIndicator v-if="sport.id === sportId" layout-id="sport-nav"
          /></RouterLink>
        </nav>
        <div class="header-actions">
          ...
          >
            ...
            <AnimatePresence>
              <motion.div
                v-if="focused && query.trim().length >= 2"
                class="search-menu"
                :initial="{ opacity: 0, y: -8, scale: 0.98 }"
                :animate="{ opacity: 1, y: 0, scale: 1 }"
                :exit="{ opacity: 0, y: -5, scale: 0.99 }"
              >
                <span v-if="searching" class="search-state"
                  >SEARCHING OFFICIAL MLB DATA…</span
                >
                <template v-else-if="quickResults.length"
                  ><button
                    ...
                    type="button"
                    @pointerdown.prevent="pickResult(item)"
                  >
                    <TeamLogo
                      v-if="item.group === 'Teams'"
                      :team="item"
                      :size="38"
                    /><PlayerHeadshot
                      v-else-if="item.group === 'Players'"
                      :id="item.id"
                      :name="item.name"
                      :size="38"
                    /><i v-else>{{ item.abbr }}</i
                    ><span
                      ><b>{{ item.name }}</b
                ...
                >
                <span v-else class="search-state"
                  >NO MATCHES IN OFFICIAL MLB DATA</span
                >
              </motion.div>
            </AnimatePresence>
          </form>
          ...
          <button
            ...
            type="button"
            :aria-label="
              store.theme === 'dark' ? 'Use light theme' : 'Use dark theme'
            "
            @click="store.toggleTheme()"
          >
            <Sun v-if="store.theme === 'dark'" /><Moon v-else />
          </button>
          <button
            class="sport-menu icon-only"
            type="button"
            :aria-expanded="sportOpen"
            aria-label="Choose sport"
            @click="sportOpen = !sportOpen"
          >
            <X v-if="sportOpen" /><Menu v-else />
          </button>
        </div>
      </div>
      ...
      <div class="context-row">
        <div class="workspace-title">
          <small>{{
            activeSport ? activeSport.eyebrow : "DECISION SYSTEM"
          }}</small
          ><b>{{ activeSport ? activeSport.name : "All sports" }}</b
          ><em v-if="activeSport">{{
            activeSport.status === "live" ? "LIVE ANALYTICS" : "MODEL LAB"
          }}</em>
        </div>
        <nav
          class="context-nav"
        ...
        >
          <RouterLink
            ...
            :key="item[1]"
            :to="item[1]"
            :class="{ active: isNavActive(item[1]) }"
            >{{ item[0]
            }}<SharedIndicator
              v-if="isNavActive(item[1])"
              layout-id="context-nav"
          /></RouterLink>
        </nav>
        <button
          v-if="nav.length"
          class="nav-menu icon-only"
          type="button"
          :aria-expanded="navOpen"
          aria-label="Open workspace navigation"
          @click="navOpen = !navOpen"
        >
          <X v-if="navOpen" /><Menu v-else />
        </button>
      </div>
      ...
      <div v-if="isBaseball" class="score-strip">
        ...
        ><RouterLink
          ...
          :to="gamePath(game)"
          ><b>{{ game.away.abbr }}</b
          ><em>{{ game.away.score }}</em
          ><span>—</span><em>{{ game.home.score }}</em
          ><b>{{ game.home.abbr }}</b
          ><small>{{ tickerState(game) }}</small></RouterLink
        ><span v-if="!tickerGames.length" class="empty-strip"
          >Official scoreboard syncing</span
        ><span class="feed-state"><i></i> FEEDS CONNECTED</span>
      </div>
    </header>
    ...
    <AnimatePresence>
      <motion.aside
        ...
        aria-label="Sport workspace menu"
      >
        ...
        <nav>
          <RouterLink to="/"
            ><small>00</small><b>All sports</b
            ><span>Combined decision system</span></RouterLink
          ><RouterLink
            v-for="sport in sports"
            :key="sport.id"
            :to="sport.route"
            :style="{ '--sport': sport.accent }"
            ><small>{{ sport.numeral }}</small
            ><b>{{ sport.name }}</b
            ><span>{{ sport.eyebrow }}</span></RouterLink
          >
        </nav>
...
.scroll-progress {
  ...
  box-shadow: 0 0 18px
    color-mix(in srgb, var(--workspace-accent) 65%, transparent);
}
.workspace-header {
  position: sticky;
  top: 0;
...
.workspace-title em {
  padding: 6px 8px;
  background: color-mix(in srgb, var(--workspace-accent) 13%, transparent);
...
.drawer-head {
  ...
  border-bottom: 1px solid #2c342d;
  color: #aab3a9;
  font: 600 12px "DM Mono";
}
.drawer-head button {
