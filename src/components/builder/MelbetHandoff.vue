...
import { Ban, Check, Copy, ExternalLink, Send, WandSparkles, X } from "lucide-vue-next";
...
function checkHelper() {
  ...
  helperMessage.value = "Checking the NINTH helper and its background service...";
  window.postMessage({ source: "NINTH_APP", type: "NINTH_MELBET_HELPER_STATUS_REQUEST" }, window.location.origin);
  helperTimeout = window.setTimeout(() => {
    if (helperState.value !== "checking") return;
    helperState.value = "missing";
    helperMessage.value = "Helper not connected. Reload NINTH MelBet Helper in chrome://extensions, then refresh this NINTH tab.";
  }, 2500);
}
...
function receiveHelperStatus(event) {
  ...
  window.clearTimeout(helperTimeout);
  if (detail.helperVersion) helperVersion.value = String(detail.helperVersion);
  helperState.value = detail.state || "ready";
  if (detail.sessionId) helperSessionId.value = String(detail.sessionId);
  if (["cancelled", "done", "error"].includes(helperState.value)) helperSessionId.value = "";
...
function autofill() {
  if (!autofillReady.value || ["connecting", "detected", "started", "working", "cancelling"].includes(helperState.value)) return;
  window.clearTimeout(helperTimeout);
  helperRequestId = globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  helperState.value = "connecting";
  helperMessage.value = "Connecting to the NINTH browser helper and opening one MelBet tab...";
  helperTimeout = window.setTimeout(() => {
      ...
      payload: {
        ...
        entries: props.entries.map(({ key, game, selection, url, automation }) => ({
          ...
          automation: automation ? {
            ...
            melbetMarketLabel: String(automation.melbetMarketLabel || ""),
            melbetPlayerName: String(automation.melbetPlayerName || ""),
            melbetSelectionName: String(automation.melbetSelectionName || ""),
            melbetDisplayLine: automation.melbetDisplayLine == null ? null : Number(automation.melbetDisplayLine),
            melbetFormat: String(automation.melbetFormat || ""),
            melbetGroupId: automation.melbetGroupId == null ? null : Number(automation.melbetGroupId),
            melbetTypeId: automation.melbetTypeId == null ? null : Number(automation.melbetTypeId),
          } : null,
        })),
      },
...
function cancelAutofill() {
  if (!["connecting", "detected", "started", "working", "cancelling"].includes(helperState.value)) return;
  window.clearTimeout(helperTimeout);
  helperState.value = "cancelling";
  helperMessage.value = "Stopping the helper before its next browser action...";
  window.postMessage({
    source: "NINTH_APP",
    type: "NINTH_MELBET_AUTOFILL_CANCEL",
    requestId: helperRequestId,
    sessionId: helperSessionId.value,
  }, window.location.origin);
}
    ...
    <div v-if="open" class="handoff-backdrop" @click.self="open = false">
      <section class="handoff-modal" role="dialog" aria-modal="true" aria-label="MelBet handoff">
        ...
        <div v-if="autofillEnabled" class="autofill-panel" :class="{ unavailable: !autofillReady }">
          <div><b>{{ playerAutofill ? 'ONE-CLICK PLAYER PROPS' : 'ONE-CLICK MELBET CARD' }}{{ helperVersion ? ` · HELPER v${helperVersion}` : '' }}</b><span>{{ autofillUnavailableReason || helperMessage || (playerAutofill ? 'Exact event, player, market, side and line are checked again immediately before every click.' : 'Moneylines are matched by exact event and W1/W2. Totals are matched under Regular time by exact side and line.') }}</span></div>
          <button v-if="['connecting', 'detected', 'started', 'working', 'cancelling'].includes(helperState)" class="cancel-autofill" type="button" :disabled="helperState === 'cancelling'" @click="cancelAutofill"><Ban /> {{ helperState === 'cancelling' ? 'STOPPING' : 'STOP AUTOFILL' }}</button>
          <button v-else type="button" :disabled="!autofillReady" @click="autofill"><WandSparkles /> {{ !autofillReady ? 'AUTOFILL UNAVAILABLE' : helperState === 'done' ? 'AUTOFILL AGAIN' : 'AUTOFILL ALL' }}</button>
        </div>
...
.autofill-panel .cancel-autofill{border-color:#d55850;background:#d55850;color:white}.autofill-panel .cancel-autofill:disabled{cursor:wait}
</style>
