...
import { AlertTriangle, LockKeyhole, RefreshCw, ShieldCheck } from 'lucide-vue-next'
...
const scheduled = computed(() => games.value.filter(row => row.status !== 'Completed'))
const marketOptions = [
...
const load = async () => {
  ...
  try {
    ...
    error.value = caught?.message || 'The NBA builder workspace could not be loaded.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="unified-builder nba-builder">
    <UnifiedBuilderHero eyebrow="NINTH / NBA BUILDER" title="Build from" accent="possession truth." description="The NBA workspace now follows MLB’s compact card-building hierarchy while retaining a hard evidence lock until exact-event moneyline and total-points forecasts are exported.">
    ...
    <section class="builder-board-note"><span><i></i>{{scheduled.length}} CURRENT NBA GAMES · 0 ELIGIBLE LINES</span><p>The schedule is operational. Selection controls unlock automatically only after the exact-event prediction exporter and live promotion gate both pass.</p></section>

    <section class="builder-day"><header><div><span class="eyebrow">CURRENT NBA BOARD</span><h2>{{scheduled.length}} scheduled games</h2></div><small>PROJECTIONS LOCKED</small></header>
      <div v-if="loading" class="builder-state"><RefreshCw class="spin"/><b>Joining the NBA slate and audit state</b><p>Loading scheduled games, model artifacts and release eligibility.</p></div>
      <div v-else-if="error" class="builder-state"><AlertTriangle/><b>NBA board unavailable</b><p>{{error}}</p></div>
      <div v-else-if="!scheduled.length" class="builder-state"><ShieldCheck/><b>No NBA fixture is currently open</b><p>The builder will populate from the next schedule sync.</p></div>
      <LayoutGroup v-else id="nba-builder"><motion.div layout class="builder-card-grid"><AnimatePresence mode="popLayout"><motion.article v-for="game in scheduled" :key="game.id" layout class="builder-choice-card nba-locked-card" :initial="reduced?false:{opacity:0,y:12}" :animate="{opacity:1,y:0}" :exit="reduced?undefined:{opacity:0,scale:.98}" :while-hover="reduced?undefined:{y:-2}">
        <div class="builder-card-meta"><span>{{game.date}} · {{game.time}}</span><strong>PROJECTION EXPORT PENDING</strong></div>
...
<style scoped>
.nba-builder{--sport:#f6b945}.builder-control-bar{grid-template-columns:minmax(210px,1fr) minmax(130px,.65fr) auto}.nba-locked-card{background:var(--surface)}.nba-locked-card .builder-selection{opacity:.62}@media(max-width:700px){.builder-control-bar{grid-template-columns:1fr 1fr}.builder-refresh{grid-column:1/-1}}@media(max-width:480px){.builder-control-bar{grid-template-columns:1fr}.builder-refresh{grid-column:1}}
</style>
