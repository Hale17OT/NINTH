...
const target = ref(5), minimumOdds = ref('all'), selected = ref([])
...
const marketDefinitions = prediction => {
  ...
  values.push({ market:'moneyline', side:home>=away?'home':'away', probability:Math.max(home,away), odds:Number(home>=away?prediction.prices?.home_moneyline:prediction.prices?.away_moneyline)||null, eligible:prediction.market_eligibility?.moneyline===true, consensus:prediction.model_consensus?.moneyline!==false })
  if(Number.isFinite(spread)&&prediction.markets?.home_spread!=null){const h=Number(prediction.markets.home_spread),a=Number(prediction.markets.away_spread);values.push({market:'spread',side:h>=a?'home':'away',line:spread,probability:Math.max(h,a),odds:Number(h>=a?prediction.prices?.home_spread:prediction.prices?.away_spread)||null,eligible:prediction.market_eligibility?.spread===true,consensus:true})}
  if(Number.isFinite(line)&&prediction.markets?.over_total!=null){const over=Number(prediction.markets.over_total),under=Number(prediction.markets.under_total);values.push({market:'total',side:over>=under?'over':'under',line,probability:Math.max(over,under),odds:Number(over>=under?prediction.prices?.over_total:prediction.prices?.under_total)||null,eligible:prediction.market_eligibility?.total===true,consensus:true})}
  return values
}
...
const candidates = computed(() => allCandidates.value.filter(row=>marketMode.value==='mixed'||row.option.market===marketMode.value).filter(row=>evidenceMode.value!=='production'||row.option.eligible).filter(row=>minimumOdds.value==='all'||(row.option.odds&&row.option.odds>=Number(minimumOdds.value))).sort((a,b)=>b.option.probability-a.option.probability))
...
const evidenceOptions=[{value:'shadow',label:'All shadow observations',meta:'Research'},{value:'production',label:'Automatic eligible only',meta:'Audited'}]
...
const weekOptions=computed(()=>[...availableWeeks.value.map(value=>({value,label:`Week ${value}`})),{value:'all',label:'All upcoming weeks'}])
...
<template>
  <div class="unified-builder nfl-builder">
    <UnifiedBuilderHero eyebrow="NINTH / NFL BUILDER" title="Build today's" accent="NFL card." description="Moneyline, spread and total-points decisions use the same compact card-building flow as MLB while every NFL market retains its own evidence gate.">
      ...
      <div class="builder-control-bar"><CustomSelect v-model="evidenceMode" label="Evidence" :options="evidenceOptions"/><CustomSelect v-model="selectedWeek" label="Slate" :options="weekOptions"/><CustomSelect v-model="target" label="Target legs" :options="targetOptions"/><CustomSelect v-model="minimumOdds" label="Odds floor" :options="oddsOptions"/><button class="builder-refresh" :disabled="loading" @click="load"><RefreshCw :class="{spin:loading}"/> REFRESH</button></div>
    </UnifiedBuilderHero>
    <UnifiedBuilderScore :probability="jointProbability" :selected="selected.length" :target="Number(target)" :average="averageProbability" :title="selected.length ? `${selected.length}-leg NFL card` : 'No NFL legs selected'" description="The displayed joint estimate multiplies the selected NFL model probabilities. One selection is allowed per game and mixed cards cap repeated market-side exposure." :detail="combinedOdds ? `Current combined decimal price ${combinedOdds.toFixed(2)}. Prices filter and describe selections; they are not model inputs.` : 'Current prices appear when the source feed lists them.'" fourth-label="MODE" :fourth-value="evidenceMode==='production'?'AUTO':'SHADOW'">
    ...
    <section class="builder-model-strip"><article v-for="market in ['moneyline','spread','total']" :key="market" :class="{ready:readiness.historical?.[market]}"><span>{{marketLabel(market)}}</span><b>{{readiness.historical?.[market]?'HISTORICAL READY':'SHADOW LOCKED'}}</b><small>{{readiness.live?.[market]?.samples||0}} / 30 LIVE · BRIER {{readiness.live?.[market]?.brier?.toFixed(3)||'—'}}</small></article></section>
    <section class="builder-board-note"><span><i></i>{{candidates.length}} CURRENT NFL LINES · WEEK {{selectedWeek}}</span><p>Production mode exposes only markets that pass their live gate. Shadow mode remains research-only and never triggers sportsbook autofill.</p></section>
    ...
    <section class="builder-day"><header><div><span class="eyebrow">CURRENT NFL BOARD</span><h2>{{candidates.length}} decision-ready lines</h2></div><small>{{scheduled.length}} GAMES</small></header>
      ...
      <div v-else-if="!candidates.length" class="builder-state"><LockKeyhole/><b>No markets pass these controls</b><p>Switch to shadow observations or widen the odds floor to inspect the available research board.</p></div>
      <LayoutGroup v-else id="nfl-builder"><motion.div layout class="builder-card-grid"><AnimatePresence mode="popLayout"><motion.article v-for="row in candidates" :key="row.id" layout class="builder-choice-card" :class="{selected:selectedIds.has(row.id)}" :initial="reduced?false:{opacity:0,y:12}" :animate="{opacity:1,y:0}" :exit="reduced?undefined:{opacity:0,scale:.98}" :while-hover="reduced?undefined:{y:-2}">
        <div class="builder-card-meta"><span>W{{weekNumber(row.game)}} · {{row.game.date}}</span><strong>{{row.option.eligible?'AUTO ELIGIBLE':'SHADOW ONLY'}}</strong></div>
        ...
        <button class="builder-selection" :class="{active:selectedIds.has(row.id)}" :disabled="(!selectedIds.has(row.id)&&occupied.has(String(row.game.id)))||(!selectedIds.has(row.id)&&selected.length>=Number(target))" @click="toggle(row)"><span><small>{{row.option.side.toUpperCase()}} · {{row.option.odds?`@ ${row.option.odds.toFixed(3)}`:'PRICE PENDING'}}</small><b>{{selectionLabel(row)}}</b></span><strong>{{formatProbability(row.option.probability)}}</strong><Check v-if="selectedIds.has(row.id)"/><Plus v-else/></button>
        <footer class="builder-card-footer"><span>{{row.option.consensus?'MODEL CONSISTENT':'MODEL DISAGREEMENT'}}</span><b>ONE PICK PER GAME</b></footer>
      </motion.article></AnimatePresence></motion.div></LayoutGroup>
    </section>
  </div>
</template>

<style scoped>.nfl-builder{--sport:#83a8ff}.builder-control-bar :deep(.custom-select){min-width:0}.builder-control-bar :deep(.select-label){font-size:11px}.builder-control-bar :deep(.trigger){min-height:48px}</style>
