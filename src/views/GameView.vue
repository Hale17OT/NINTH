...
onMounted(()=>{load();clockTimer=window.setInterval(()=>{now.value=Date.now()},1000)})
onBeforeUnmount(() => { window.clearTimeout(refreshTimer); window.clearInterval(clockTimer) })
  ...
  <div v-if="game" class="view">
    ...
    <SectionCard :title="isFinal ? 'Final model review' : 'Market-free moneyline projection'" :subtitle="isFinal ? 'Original forecast compared with the official final result' : 'Calibrated from baseball data only · no odds or sportsbook inputs'">
      <div v-if="game.projection?.available" class="projection">
        ...
        <div class="probability-row"><div><TeamLogo :team="game.away" :size="48"/><span><small>{{ game.away.abbr }} {{ isFinal ? 'FORECAST' : 'WIN' }}</small><strong class="mono"><AnimatedNumber :value="game.projection.away_win_probability*100" suffix="%"/></strong></span></div><div class="model-pick"><small>{{ isFinal ? 'ORIGINAL MODEL PICK' : 'PROJECTED SIDE' }}</small><b>{{ projectedTeam?.name }}</b><span>{{ isFinal ? 'Archived forecast · not a postgame chance' : 'Decision support · not a guarantee' }}</span></div><div class="home-prob"><span><small>{{ game.home.abbr }} {{ isFinal ? 'FORECAST' : 'WIN' }}</small><strong class="mono"><AnimatedNumber :value="game.projection.home_win_probability*100" suffix="%"/></strong></span><TeamLogo :team="game.home" :size="48"/></div></div>
        ...
        <div class="reason-grid"><article v-for="reason in game.projection.reasons" :key="reason.feature" :class="reason.direction"><span :class="reason.direction" :aria-label="`${reason.direction} team ${reason.direction === 'home' ? game.home.abbr : game.away.abbr}`">{{ reason.direction === 'home' ? game.home.abbr : game.away.abbr }}</span><div><b>{{ reason.label }}</b><small>{{ reason.direction }} projection shift · model impact {{ Math.abs(reason.impact).toFixed(2) }}</small></div></article></div>
