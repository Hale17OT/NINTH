...
import { moneylineBuilderProbability } from '../services/slipBuilderRecommendations'
...
const sportState = sport => sport.id === 'baseball' ? 'LIVE BOARD' : researchBoards.value[sport.id]?.some(game => game.prediction) ? 'MODEL BOARD' : 'EVIDENCE REQUIRED'
const productionCandidates = computed(() => (board.value?.games || []).flatMap(game => {
  ...
  if (game.automatic_moneyline_eligible && game.recommended_side && game.recommended_probability) {
    ...
    rows.push({ id:`mlb:${game.game_id}:moneyline:${side}`, sport:'baseball', sportLabel:'MLB', gameId:game.game_id, game, event:`${game.away.name} @ ${game.home.name}`, startsAt:game.starts_at, market:'Moneyline', selection:`${team.name} to win`, probability:moneylineBuilderProbability(game), rawProbability:Number(game.recommended_probability), odds:Number(game.moneyline_odds?.[side]||0), evidence:game.moneyline_uncertainty_multiplier < 1 ? 'BUILDER ELIGIBLE · DOUBLEHEADER SHRINK' : 'BUILDER ELIGIBLE' })
  }
...
const evidenceOptions = [{value:'production',label:'Builder eligible',meta:'Audited only'},{value:'shadow',label:'All model forecasts',meta:'Full analytical board'}]
...
<template>
  <div class="unified-builder multi-builder">
    ...
    <UnifiedBuilderScore :probability="jointProbability" :selected="selected.length" :target="Number(target)" :average="averageProbability" :title="selected.length?`${selected.length}-leg combined card`:'No combined legs selected'" description="The joint score multiplies probabilities only across different events. Cross-sport builds cap concentration in any one model family." :detail="combinedOdds?`Current combined decimal price ${combinedOdds.toFixed(2)}. Analytical-only legs never become builder eligible.`:'Combined price appears only when every selected leg has a current listed price.'" fourth-label="EVIDENCE" :fourth-value="viewMode==='production'?'ELIGIBLE':'ALL'">
    ...
    <section class="builder-board-note"><span><i></i>{{visible.length}} VISIBLE LINES · {{viewMode==='production'?'BUILDER ELIGIBLE':'ALL MODEL FORECASTS'}}</span><p>Combined does not mean blended: probabilities stay attached to their sport-native model, and evidence-gated sports remain clearly labelled.</p></section>
    ...
    <section class="builder-day"><header><div><span class="eyebrow">{{viewMode==='production'?'BUILDER INVENTORY':'CROSS-SPORT MODEL INVENTORY'}}</span><h2>{{visible.length}} decision-ready outcomes</h2></div><small>{{activeSport==='all'?'ALL SPORTS':sports.find(item=>item.id===activeSport)?.name}}</small></header>
      <div v-if="loading" class="builder-state"><RefreshCw class="spin"/><b>Syncing sport adapters</b><p>Joining current immutable model boards.</p></div>
      <div v-else-if="error" class="builder-state"><AlertTriangle/><b>Combined board unavailable</b><p>{{error}}</p></div>
      <div v-else-if="viewMode==='production'&&activeSport!=='all'&&activeSport!=='baseball'" class="builder-state"><LockKeyhole/><b>{{sports.find(item=>item.id===activeSport)?.name}} is evidence-locked</b><p>Switch to all model forecasts to inspect exact-event probabilities without making them builder eligible.</p></div>
      <div v-else-if="!visible.length" class="builder-state"><AlertTriangle/><b>No outcomes pass these controls</b><p>Widen the odds floor, change sport, or inspect all model forecasts. No manual-only signal is substituted.</p></div>
