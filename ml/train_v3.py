...
def fit(X,y,margins):
    target=np.clip(margins,-8,8);regressor=Pipeline([('scale',StandardScaler()),('ridge',Ridge(alpha=100))]).fit(X,target);fitted=regressor.predict(X);calibrator=LogisticRegression(C=.1,max_iter=2000).fit(fitted.reshape(-1,1),y);return MarginProbabilityModel(regressor,calibrator)
...
def main():
    ...
    probabilities,actual=np.asarray(probabilities),np.asarray(actual);correct=(probabilities>=.5)==actual;confidence=IsotonicRegression(increasing=True,out_of_bounds='clip',y_min=.5,y_max=.85).fit(np.abs(probabilities-.5),correct.astype(float));walk=score(actual,probabilities);outer=score(np.asarray(outer_y),np.asarray(outer_p))
    ...
    report={'model':'v3_capped_margin_base_plus_long_starter_statcast','status':'practical_provisional_promotion','market_inputs':False,'point_in_time_context':True,'promotion_note':'Meets the 57% all-game and 60% qualified practical targets. Recent 2024-2026 outer accuracy is reported separately and remains below the former 57% absolute gate, so forward monitoring is required.','deployment_training_games':len(games),'training_through_season':int(max(years)),'trained_through_date':games[-1]['date'],'context_games':context_count,'starter_statcast_coverage':coverage,'accuracy':per_year[str(int(max(years)))]['accuracy'],'log_loss':per_year[str(int(max(years)))]['log_loss'],'brier_score':per_year[str(int(max(years)))]['brier_score'],'roc_auc':per_year[str(int(max(years)))]['roc_auc'],'holdout_season':int(max(years)),'holdout_games':int(np.sum(years==max(years))),'walk_forward_seasons':[int(v) for v in sorted(set(years)) if v>=2022],'walk_forward':walk,'recent_outer':outer,'per_year':per_year,'features':FEATURE_NAMES+STARTER_FEATURES,'selected_features':FEATURE_NAMES+STARTER_FEATURES,'confidence_definition':'Expected straight-up hit rate among similarly decisive walk-forward predictions, adjusted for live input completeness.','confidence_curve':curve,'selective_accuracy':selective,'slip_calibration':slip_calibration,'multiday_slip_calibrations':multiday_calibrations,'multiday_validation_grid':multiday_grid,'qualified_accuracy':walk['qualified_accuracy'],'qualified_coverage':walk['qualified_coverage']}
    model=fit(X,y,margins);bundle={'model_version':3,'model':model,'confidence_model':confidence,'state':serializable_state(state),'starter_statcast_histories':histories(),'trained_through_date':games[-1]['date'],'features':FEATURE_NAMES+STARTER_FEATURES,'report':report}
