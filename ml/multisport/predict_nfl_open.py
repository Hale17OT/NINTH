"""Generate keyless NFL shadow forecasts from nflverse schedules."""
...
def score_projection(bundle: dict, features: dict, total_line: float | None, spread_line: float | None) -> dict:
    matrix = np.asarray([[features.get(name, np.nan) for name in bundle["features"]]], dtype=float)
    ...
    return {
        ...
        "expected_home_score": max(0.0, (expected_total + expected_margin) / 2),
        "expected_away_score": max(0.0, (expected_total - expected_margin) / 2),
...
def model_readiness(artifact_dir: Path, live_audit: dict | None = None) -> dict:
    ...
    home = _read_report(artifact_dir / "home_win.json")
    score = _read_report(artifact_dir / "score.json")
    historical = {
        "moneyline": bool((home.get("historical_readiness") or {}).get("passed")),
        "spread": bool(((score.get("historical_readiness") or {}).get("spread"))),
        "total": bool(((score.get("historical_readiness") or {}).get("total"))),
    }
...
def forecast(rows: list[dict], artifact_dir: Path, now: datetime, advanced: dict | None = None, live_audit: dict | None = None) -> dict:
    ...
    for row in ordered:
        ...
        # Keep the open baseline in shadow and shrink current probabilities accordingly.
        ...
        score = score_projection(score_bundle, features, total_line, spread_line)
        ...
        upcoming.append({
            ...
            "status": "production_eligible" if any(eligible.values()) else "shadow_only", "builder_eligible": any(eligible.values()),
...
def main() -> None:
    ...
    result = forecast(rows, args.artifact_dir, datetime.now(timezone.utc), advanced_summaries(2023, datetime.now().year - 1, args.output.parent), live_audit)
