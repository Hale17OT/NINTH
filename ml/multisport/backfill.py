...
def main() -> None:
    ...
    if args.refresh_sources and "football" in selected:
        ...
        run(sys.executable, "-m", "ml.multisport.collect_football_open", "--start-season", str(current_year - 6), "--end-season", str(current_year - 1))
    ...
    for sport in selected - {"esports"}:
        for path in (DATA / sport).glob("*.jsonl"):
            if path.name in {"raw_matches.jsonl", "over_total.jsonl"}:
                continue
